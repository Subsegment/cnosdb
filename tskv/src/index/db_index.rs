...
impl DBIndex {
    ...
    pub fn add_series_if_not_exists(&self, info: &Point) -> IndexResult<u64> {
        ...
        //if not exist add it!
        //todo: use a memory atomic val to replace
    ...
    //     match self.table_schema.write().get_mut(info.table()) {
    ...
    //                     self.table_schema.write().insert(info.table().to_string(), list);
    //                     schema = self.table_schema.write().get_mut(info.table()).unwrap();
    //                 }
    //             }
    //         }
    //     }
